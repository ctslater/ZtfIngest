...
object ZtfIngest {
  ...
  def deduplicatePartition(zone: Long,
    ...
    val matches = new ListBuffer[(Long, SourceId, SourceId, Double, Double)]
      ...
      case MatchWithCoordZone(_, id1, _, _, id2, ra2, dec2) =>
        if(primaryIds.contains(id1)) {
          if(! matchedIds.contains(id2)) {
            matches.append((zone, id1, id2, ra2, dec2))
            matchedIds += (id2 -> id1)
          }
        } else if (primaryIds.contains(id2)) {
            if(! matchedIds.contains(id1)) {
              matches.append((zone, id2, id1, ra2, dec2))
              matchedIds += (id1 -> id2)
            }
        } else {
          if (matchedIds.contains(id1)) {
            matches.append((zone, matchedIds(id1), id2, ra2, dec2))
            matchedIds += (id2 -> matchedIds(id1))
          } else if (matchedIds.contains(id2)) {
            matches.append((zone, matchedIds(id2), id1, ra2, dec2))
